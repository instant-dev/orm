...
class MigrationManagerDangerousFilesystem {
  ...
  write (migration) {
    ...
    if (filesystemMigrations.find(json => json.id === migrationJSON.id)) {
      throw new Error(`Can not write migration to "${this.self.parent._Schema.migrationsDirectory}": migration with (id=${migrationJSON.id}) already exists`);
    }
    const filename = Migration.generateFilename(migrationJSON.id, migrationJSON.name);
    const fullpath = path.join(this.self.parent._Schema.migrationsDirectory, filename);
    const buffer = Buffer.from(JSON.stringify(migrationJSON, null, 2));
    if (!fs.existsSync(this.self.parent._Schema.migrationsDirectory)) {
      fs.mkdirSync(this.self.parent._Schema.migrationsDirectory);
    }
    if (fs.existsSync(fullpath)) {
      throw new Error(`Can not write migration to "${this.self.parent._Schema.migrationsDirectory}": file already exists`);
    } else if (!fs.statSync(this.self.parent._Schema.migrationsDirectory).isDirectory()) {
      throw new Error(`Can not write migration to "${this.self.parent._Schema.migrationsDirectory}": not a directory`);
    }
  ...
  rewind (steps = 1) {
    ...
    if (!migrations.length) {
      ...
      for (let i = 0; i < steps; i++) {
        ...
        try {
          fs.unlinkSync(path.join(this.self.parent._Schema.migrationsDirectory, filename));
