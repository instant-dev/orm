...
class InstantORM extends Logger {
  ...
  /**
   * Reads environment variables from appropriate .[environment].env into an object
   * @param {string} pathname
   * @returns {object} envVars
   */
  readEnvObject (pathname) {
    const entries = this.readEnv(pathname);
    return entries.reduce((envVars, entry) => {
  ...
  /**
   * Reads environment variables from appropriate .[environment].env and commits them to process.env
   * @param {string} pathname
   * @returns {object} envVars
   */
  useEnvObject (pathname) {
    const envVars = this.readEnvObject(pathname);
    process.env._ORIGINAL_NODE_ENV = process.env._ORIGINAL_NODE_ENV || process.env.NODE_ENV || 'development';
